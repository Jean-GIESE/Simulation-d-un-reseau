        ...
        for (int i=0; i<nbEquipements; i++)
        {
            ...
            if (type == 2) {
                ...
                if (reseau->sommets[i].objet.sw.tabCommutation != NULL) {
                    free(reseau->sommets[i].objet.sw.tabCommutation);
                }
                reseau->sommets[i].objet.sw.tabCommutation = malloc(sizeof(Commutation) * nbPorts);

                if (reseau->sommets[i].objet.sw.etat_ports != NULL) {
                    free(reseau->sommets[i].objet.sw.etat_ports);
                }
                ...
                if ((reseau->sommets[i].objet.sw.tabCommutation != NULL) && (reseau->sommets[i].objet.sw.etat_ports != NULL)) {
                    for (size_t j = 0; j < nbPorts; j++) {
                        ...
                        reseau->sommets[i].objet.sw.etat_ports[j] = DESACTIVE;
                    }
                }
        ...
        while (fgets(ligne, sizeof(ligne), fichier)) {
            ...
            if (reseau->liens[i].s1->type == TYPE_SWITCH) {
                ...
                if (sw1->ports_utilises < sw1->nb_ports) {
                    size_t port = sw1->ports_utilises++;
                    reseau->liens[i].port_s1 = port;
                    sw1->etat_ports[port] = ACTIF;
                } else {
            ...
            if (reseau->liens[i].s2->type == TYPE_SWITCH) {
                ...
                if (sw2->ports_utilises < sw2->nb_ports) {
                            size_t port = sw2->ports_utilises++;
                            reseau->liens[i].port_s2 = port;
                            sw2->etat_ports[port] = ACTIF;
                } else {
                    fprintf(stderr, "Plus de ports disponibles sur %s\n", sw2->nom);
                    exit(1);
                }
...
void afficher_switch(const Switch *sw) {
    ...
    printf("  Priorite : %u\n", sw->priorite);
    printf("  Capacite : %zu\n", sw->capacite);
    printf("  Ports utilisés : %zu\n", sw->ports_utilises);

    if (sw->etat_ports && sw->nb_ports > 0) {
        printf("  Etats des ports :\n");
        for (size_t i = 0; i < sw->nb_ports; i++) {
            const char *etat_str = NULL;
            switch (sw->etat_ports[i]) {
                case DESACTIVE: etat_str = "DESACTIVE"; break;
                case ACTIF:    etat_str = "ACTIF";     break;
                case BLOQUE:   etat_str = "BLOQUE";    break;
                // Ajoute ici d'autres états si définis
                default:        etat_str = "INCONNU";  break;
            }
            printf("    Port %zu : %s\n", i, etat_str);
        }
    } else {
        printf("  Etats des ports : non initialisés\n");
    }
